...
import 'package:yedi_app/pages/home/advertiser/adverts/advertiser_adverts_content.dart';
...
class AdvertiserAdvertsView extends StatelessWidget {
  const AdvertiserAdvertsView({super.key});

  @override
  Widget build(BuildContext context) {
    final tabController =
        context.watch<AdvertiserAdvertsTabControllerCubit>().state;
      ...
      children: [
        ...
        Expanded(
          child: TabBarView(controller: tabController, children: [
            BlocBuilder<ListAdvertiserDayToDayAdvertsBloc, ListAdvertsState>(
                buildWhen: (previous, current) =>
                    previous.status != current.status ||
                    previous.adverts.length != current.adverts.length,
                builder: (context, state) =>
                    _blocBuilder<ListAdvertiserDayToDayAdvertsBloc>(
                        context, state, AdvertType.day_to_day)),
            BlocBuilder<ListAdvertiserLongTermAdvertsBloc, ListAdvertsState>(
                buildWhen: (previous, current) =>
                    previous.status != current.status ||
                    previous.adverts.length != current.adverts.length,
                builder: (context, state) =>
                    _blocBuilder<ListAdvertiserLongTermAdvertsBloc>(
  ...
  _blocBuilder<T extends StateStreamable<ListAdvertsState>>(
    ...
    switch (state.status) {
      ...
      case ListAdvertsStatus.loaded:
        if (state.adverts.isEmpty) {
          ...
          body = AdvertiserAdvertsContent<T>(advertType: advertType);
        }
    }
    ...
    return SingleChildScrollView(
        ...
        child:
          ...
          Text(
            advertType == AdvertType.day_to_day
                ? AppLocalizations.of(context)!.dayToDayExplanationAdvertisers
                : AppLocalizations.of(context)!.longTermExplanationAdvertisers,
            textAlign: TextAlign.center,
            style: GoogleFonts.sora(
              fontSize: 14,
              height: 24 / 14,
